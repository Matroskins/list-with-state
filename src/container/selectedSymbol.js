...
let SelectedSymbol = ({ selectState, onClick }) => {
    let selectedIcon = (selectState) ? { classNameSelected: 'selectedElement', iconName: 'fa-check-circle', buttonText: 'Selected' } : { classNameSelected: 'unSelectedElement', iconName: 'fa-plus-circle', buttonText: 'Unselected' }
    return ( <
        ...
        <
        div > <
        button className = 'selected__hover' > { selectedIcon.buttonText } < /button> </div > <
        i className = { 'selectedIcon ' + ' fa ' + selectedIcon.iconName } > < /i>

        <
        div > <
        button className = 'unSelected__hover' > { selectedIcon.buttonText } <
        /button> </div > <
